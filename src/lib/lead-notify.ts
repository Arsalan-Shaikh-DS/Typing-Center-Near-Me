...
export async function sendLeadEmail(subject: string, payload: LeadPayload) {
  ...
  const rows = Object.entries(payload)
    ...
    .map(
      ([k, v]) =>
        `<tr><td style="padding:6px 12px;font-weight:600;color:#0b2e59;border-bottom:1px solid #eee;">${escapeHtml(
          k,
  ...
  await transporter.sendMail({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:520px;">
        <h2 style="color:#0b2e59;">${escapeHtml(subject)}</h2>
 ...
 *                               Apps Script as a Web App (see README.md)
 *
