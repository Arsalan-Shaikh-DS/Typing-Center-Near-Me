...
/**
 * Builds the lightweight (name + phone only) schema used by the mini
 * enquiry form embedded on service/location pages and the site-wide
 * enquiry popup. Phone is capped at 10 digits, same rule as the main form.
 */
export function createMiniEnquiryFormSchema(locale: string) {
  const isAr = locale === "ar";
  return z.object({
    name: z.string().min(2, isAr ? "يرجى إدخال اسمك الكامل" : "Please enter your full name"),
    phone: z
...
export type MiniEnquiryFormValues = z.infer<ReturnType<typeof createMiniEnquiryFormSchema>>;
